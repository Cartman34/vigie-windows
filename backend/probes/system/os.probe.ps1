...
New-ModuleObject -Id 'os' -Theme 'system' -Label 'Windows' -Status $(if ($activated) {'ok'} else {'warn'}) -Fields @(
    New-Field -Key 'edition'   -Label 'Édition'    -Value $caption    -Kind 'text' -Status $(if ($isPro) {'ok'} else {'neutral'}) -Help 'Édition de Windows installee (Pro attendu).'
    New-Field -Key 'activated' -Label 'Activation' -Value $activated  -Kind 'bool' -Status $(if ($activated) {'ok'} else {'warn'})    -Help 'Windows est active (licence valide).'
    New-Field -Key 'build'     -Label 'Version'    -Value $build      -Kind 'text' -Status 'neutral'                                 -Help 'Version et numero de build de Windows.'
)
