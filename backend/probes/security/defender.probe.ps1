...
if (-not $avs) {
    New-ModuleObject -Id 'antivirus' -Theme 'security' -Label 'Antivirus' -Status 'neutral' -Fields @(
        New-Field -Key 'status' -Label 'Etat' -Value 'indisponible' -Kind 'text' -Status 'neutral' -Help "Centre de sécurité Windows non interrogeable sur ce système."
    )
    return
}
...
$fields = @(
    ...
    New-Field -Key 'upToDate' -Label 'A jour'       -Value ([bool]$primary.upToDate) -Kind 'bool' -Status $(if ($primary.upToDate) {'ok'} else {'warn'}) `
...
if ($others.Count -gt 0) {
    $fields += New-Field -Key 'others' -Label 'Autres détectés' -Value (($others | ForEach-Object { $_.name }) -join ', ') -Kind 'text' -Status 'neutral' -Help "Autres antivirus enregistres (souvent Windows Defender en veille)."
}
