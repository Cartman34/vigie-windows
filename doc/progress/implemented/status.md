...
| CORE-LIFECYCLE | À faire | Inventaire : [components.md](components.md) | Aucune pièce n'est encore passée entièrement au crible. Écarts relevés le 13/09 : le clone du service bloqué par des étiquettes déplacées et sans action de maintenance ; la source du journal d'événements et le droit de session du compte de service jamais retirés ; les journaux jamais purgés ; les déclarations `safe.directory` qui s'accumulent. **S15**, **D112** |
