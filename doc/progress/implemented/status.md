...
| CORE-OPERATIONS | Partiel | `Start-Operation` pour toute opération longue lancée par une action : marque d'occupation posée avant la réponse, résultat écrit à toute fin, processus disparu sans résultat affiché en échec ; `/operations` interrogé par toutes les pages ; inventaire : [operations.md](operations.md), tenu par `check-operations` | **Éprouvé en production le 13/09 à 11 h 57** sur `vigie-update` : la réponse de l'action portait déjà l'opération en cours. Pas encore éprouvé sur Windows Update, le disque ni les paquets. Le recalcul d'une sonde périmée reste hors protocole, non arbitré. **S14**, **D82** |
...
| UI-ACTION-TRACK | Partiel | Suivi d'action, ligne rouge en cas d'échec, notification | La réponse d'une opération longue porte l'opération en cours depuis le 13/09, constaté sur `vigie-update` ; l'affichage de la page n'a pas été observé (**S14**) |
