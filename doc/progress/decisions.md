...
> Règle : **seuls les faits marquants** sont consignés ici, avec leur support éventuel (maquette, paramètres exacts) dans le dépôt.
>
> **Ce qui mérite une décision.** Une entrée qui recopie le plan cible ou la documentation n'a pas sa place ici : ce serait
> une seconde copie, vite fausse. Avant d'écrire une entrée :
>
> - **Est-ce un choix majeur, déterminant pour l'application ?** Sinon, ce n'est pas une décision.
> - **Le choix est-il déjà déterminé ?** Alors il n'est pas nouveau. Il manque peut-être de la documentation : c'est elle
>   qu'on complète.
> - **Un choix de technologie contre une autre** est important.
> - **Le style d'affichage d'un message** ne l'est pas.
> - **Un choix d'algorithme non déterminant** ne l'est pas. **Un algorithme donné par l'opérateur, ou déterminant**, l'est
>   très probablement.
> - **Un choix déjà fait qui a posé problème** est important : jugé peu important au final, source d'un conflit entre
>   décisions, ou cause d'un défaut dans l'application. On le consigne sur l'entrée concernée quand elle existe.
>
> La liste n'est pas close : elle se complète quand un cas nouveau se présente.
...
**Constat du 12/09/2026.** La règle vaut pour chaque action longue, mais elle n'a été appliquée qu'aux deux actions du
jour et n'a jamais été reportée dans le plan cible. Quatre opérations longues sont restées hors du protocole, et une
installation de mises à jour Windows s'est annoncée terminée dès son départ. Le protocole généralisé vit dans
`targeting/operations.md`. Preuve : `notes/evidence/2026-09-12-operations-outside-the-protocol.md`.
