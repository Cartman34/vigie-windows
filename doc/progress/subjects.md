...
| **S13** | La grosse icone des bulles est celle de Windows, pas celle de Vigie | `apps/tray/tray.ps1` | Une bulle `NotifyIcon` ne choisit son glyphe que parmi `Info`/`Warning`/`Error` : le triangle jaune vu le 09/09 est celui du systeme, alors que Vigie livre ses propres `ok.ico`, `warn.ico`, `error.ico`. Il faut sortir de `ShowBalloonTip` — soit `Shell_NotifyIcon` avec `NIIF_USER`, soit la vraie API de notification, qui rendrait aussi des boutons d'action possibles. L'identite `Sowapps.Vigie` (S11) est deja posee, c'est ce qu'elle demande. |
