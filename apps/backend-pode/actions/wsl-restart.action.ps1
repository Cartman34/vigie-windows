...
if ($running) { @{ message = 'WSL redémarré.'; result = @{ ok = $true; invalidate = @('wsl.probe.ps1') } } }
else          { @{ message = 'WSL redémarrage : état non confirmé.'; result = @{ ok = $false; invalidate = @('wsl.probe.ps1') } } }
