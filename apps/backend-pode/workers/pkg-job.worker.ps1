...
try {
    if ($op -eq 'upgrade') {
        ...
        try { Write-Log -Backend $Backend -Name 'pkgupgrade' -Message ("$mgr : exit=$($up.exit) ok=$($up.ok) reboot=$($up.reboot)") } catch { }
        try {
    ...
    $etat = @{ count = [int]$u.count; items = @($u.items); at = (Get-Date).ToString('s') }
